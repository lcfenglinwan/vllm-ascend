...
def get_linear_quant_type_mix(quant_description: Dict[str, Any], prefix: str,
                              packed_modules_mapping: Dict[str, Any]):
    proj_name = prefix.split(".")[-1]
    all_quant_types = dict()
    if proj_name in packed_modules_mapping:
        quant_type = None
        shard_prefixes = [
        ...
        for shard_prefix in shard_prefixes:
            ...
            if quant_type is None:
                quant_type = shard_quant_type
            all_quant_types[shard_prefix] = shard_quant_type
    else:
        quant_type = quant_description[prefix + '.weight']
    if len(set(list(all_quant_types.values()))) > 1:
        return all_quant_types
    return quant_type
...
def get_quant_type_for_layer(
    ...
    # Linear / MoE
    return get_linear_quant_type_mix(quant_description, prefix, packed_modules_mapping)
...
def create_scheme_for_layer(
    ...
    if isinstance(quant_type, dict):
        main_quant_type = None
        for sub_linear_name, sub_quant_type in quant_type.items():
            if "up_proj" in sub_linear_name:
                main_quant_type = sub_quant_type
                
        scheme_cls = get_scheme_class(main_quant_type, layer_type)
        if scheme_cls is not None:
            return scheme_cls(quant_type)
        else:
